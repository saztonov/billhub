...
import { DownloadOutlined, EyeOutlined, EditOutlined, FileAddOutlined } from '@ant-design/icons'
...
export const invoiceAmountMask = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const clean =
    dotIdx >= 0 ? raw.slice(0, dotIdx + 1) + raw.slice(dotIdx + 1).replace(/\./g, '') : raw
      ...
      <Form form={editForm} layout="vertical" style={{ marginBottom: 16 }}>
        ...
        >
          <Descriptions.Item label="Номер">
            {extractRequestNumber(request.requestNumber)}
          </Descriptions.Item>
        ...
        <FileUploadList
          ...
          onChange={setEditFileList}
          showValidation={showEditFileValidation}
        />
      </Form>
    )
  }
      ...
      <>
        <Descriptions column={isMobile ? 1 : 2} size="small" bordered style={{ marginBottom: 16 }}>
          ...
          <Descriptions.Item label="Статус">
            ...
            >
              {request.statusName}
              {request.statusName?.startsWith('Согласование ОМТС') &&
          ...
          <Descriptions.Item label="Дата создания">
            {formatDate(request.createdAt, !isCounterpartyUser)}
          </Descriptions.Item>
    ...
    <Descriptions column={isMobile ? 1 : 2} size="small" bordered style={{ marginBottom: 16 }}>
      ...
      <Descriptions.Item label="РП">
        {actualRequest?.dpNumber ? (
          <Space size={4}>
            <span>
              №{actualRequest.dpNumber} от{' '}
              {actualRequest.dpDate
                ? new Date(actualRequest.dpDate).toLocaleDateString('ru-RU', {
                    day: '2-digit',
                    month: '2-digit',
                  })
                : '—'}
              {actualRequest.dpAmount != null &&
                `, ${actualRequest.dpAmount.toLocaleString('ru-RU', { minimumFractionDigits: 0, maximumFractionDigits: 2 })} ₽`}
            </span>
              ...
              <>
                <Tooltip title="Просмотр файла РП">
                  <Button
                    ...
                    onClick={() =>
                      setPreviewFile({
                        fileKey: actualRequest.dpFileKey!,
                        fileName: actualRequest.dpFileName ?? 'rp-file',
                        mimeType: getMimeFromFileName(actualRequest.dpFileName),
                      })
                    }
                  />
                </Tooltip>
                <Tooltip title="Скачать файл РП">
                  <Button
                    ...
                    size="small"
                    loading={downloading === actualRequest.dpFileKey}
                    onClick={() =>
                      handleDownload(
                        actualRequest.dpFileKey!,
                        actualRequest.dpFileName ?? 'rp-file',
                      )
                    }
                  />
                </Tooltip>
            ...
            {!isCounterpartyUser && !(actualRequest?.rpLinked || request.rpLinked) && (
          ...
          <Space size={8}>
            ...
            {request.approvedAt &&
              !isCounterpartyUser &&
              !(actualRequest?.rpLinked || request.rpLinked) && (
                <Button size="small" type="link" onClick={() => setDpModalOpen(true)}>
                  Заполнить
                </Button>
              )}
          </Space>
