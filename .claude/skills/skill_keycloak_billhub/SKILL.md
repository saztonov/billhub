...
- ~~bcrypt `PasswordHashProvider`~~ — **ГОТОВ** (auth `keycloak/providers/bcrypt-spi/`, доказан 2026-07-05 на тест-realm; контракт в `keycloak/providers/CREDENTIAL_CONTRACT.md`; jar инертен на su10 KC).
...
- ✅ **Сделан** protocol-mapper user-attribute **`billhub_user_id`** (access+id+userinfo) у клиента `billhub` в `auth/keycloak/realm/su10-realm.yaml`. ⚠️ **KC 26 отбрасывает неописанные атрибуты, а config-cli 6.x секцию `userProfile` НЕ применяет** (adorsys #979) — поэтому user-profile ставится **прямым Admin API PUT** (`auth/keycloak/realm/su10-userprofile.json` + `apply-userprofile.sh`, в deploy-auth.sh шаг после config-cli): `billhub_user_id` объявлен управляемым admin-only атрибутом (иначе при import/create теряется, резолв-по-claim не работает). Связка доказана на тест-realm up-poc (`verify-userprofile-poc.sh`, 2026-07-05): атрибут сохраняется, claim в токене. _(✅ применено и **доказано на ЖИВОМ su10** 2026-07-05: маппер — через config-cli, профиль — через `apply-userprofile.sh`.)_
...
- ✅ **Import-креды готовы и доказаны на su10** (2026-07-05): клиент `billhub-import` (`client_credentials`, роль `manage-realm`, `enabled:false` — включать на окно миграции) в `auth/keycloak/realm/su10-realm.yaml`; секрет `BILLHUB_IMPORT_CLIENT_SECRET` в su10 `.env`. Проверено `auth/keycloak/realm/verify-import-creds.sh`: `partialImport` под ним = HTTP 200, атрибут сохраняется. НЕ сервис-аккаунт `billhub` (у него manage-users; `partialImport` требует manage-realm).
- Сверить `OIDC_CLIENT_SECRET` billhub с секретом, который config-cli взял из `.env` (`BILLHUB_CLIENT_SECRET`).
- **Вся Предпосылка №0 (auth) выполнена и доказана на su10.** Порядок cutover и что даёт auth-сторона — `auth/deploy/billhub-migration-prep.md`.
...
- **import** батчами: `POST /admin/realms/su10/partialImport` (`ifResourceExists=SKIP`); payload на юзера: `{id:users.id, username:email, email, emailVerified:true, enabled:true, firstName, lastName, attributes:{billhub_user_id:[users.id]}, credentials:[bcrypt→{type:"password", secretData:'{"value":"<полный $2…>"}', credentialData:'{"hashIterations":<cost>,"algorithm":"bcrypt"}'}] | null-хэш→без credentials}`. `firstName`/`lastName` **обязательны** (без них VERIFY_PROFILE → вход `Account is not fully set up`) — брать из БД BillHub. Точный формат credential (JSON-строки secretData/credentialData) — `auth/keycloak/providers/CREDENTIAL_CONTRACT.md`.
...
- (auth-repo) ✅ SPI доказан на тест-realm 2026-07-05: `$2a/$2b/$2y`, cost 12/10, первый вход → argon2 (`bcrypt-spi/verify-bcrypt-poc.sh`).
