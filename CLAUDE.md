...
### СТРОГО: изоляция своего кода при коммите/пуше

- Коммитить и пушить ТОЛЬКО файлы, изменённые в рамках текущей задачи/сессии. Никогда не включать чужие изменения из параллельных сессий.
- ЗАПРЕЩЕНО `git add .`, `git add -A`, `git add -u` и любое массовое добавление. Только явное перечисление своих файлов (`git add <файл> ...`).
- Перед `git add` сверить список своих файлов с `git status`: если в рабочей копии есть изменения/новые файлы, которых ты не создавал в этой задаче (напр. другая фича), — НЕ добавлять их и предупредить разработчика.
- Если свои изменения перемешаны с чужими в одном файле — остановиться и спросить, не коммитить «как есть».
