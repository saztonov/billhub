...
Защита маршрутов: `src/components/RoleGuard.tsx`.
Фильтрация меню: `src/layout/MainLayout.tsx` (разное меню по ролям).
...
- Никогда не использовать `select('*')` в Supabase-запросах — явно перечислять поля
...
- Ограничивать размер загружаемых файлов через `VITE_MAX_FILE_SIZE_MB` (по умолчанию 100 МБ)
- Проверять тип файла по magic bytes, не только по расширению/MIME (`src/utils/fileValidation.ts`)
- НЕ использовать `console.log`/`console.warn`/`console.error` — логировать через `logError()` из `src/services/errorLogger.ts` в таблицу `error_logs` (просмотр в админке на странице логов)
- Не передавать чувствительные поля (токены, ключи) в клиентский state
...
- ВСЕГДА создавать новую миграцию с расчетом на то, что предыдущая уже запущена в Supabase
...
- Если коммит и пуш всё же выполняются (по явному запросу) — делать их всегда изолированными: коммитить/пушить строго файлы текущей задачи, без `git add -A` / `git add .`, не подхватывать несвязанные правки. Это переопределяет поведение skill `push` («все изменения»).
