--
-- 0000_baseline.sql — базовый снимок схемы public (Iteration 4, cutover-critical).
--
-- Источник: pg_dump --schema-only текущего прода (Supabase, PG 17.6), очищенный для наката
-- на не-Supabase PostgreSQL (testcontainers / Yandex Managed PG):
--   - удалены psql-метакоманды (\restrict / \unrestrict);
--   - удалены версионные SET (transaction_timeout есть только в PG17) и set_config(search_path);
--   - удалён CREATE SCHEMA public (схема уже существует на чистой БД);
--   - удалён FK users_id_fkey -> auth.users (единственная DDL-привязка к Supabase Auth;
--     в standalone-режиме users.id самостоятелен, см. план Iteration 6).
-- Функция change_user_password оставлена как есть и создаётся при SET check_function_bodies=false
-- (тело ссылается на auth.* и не валидируется при CREATE; заменяется в Iteration 6).
--
-- Снимок отражает состояние схемы ПОСЛЕ миграций 001-006. Поэтому migration-runner
-- (server/src/cli/migrate.ts) помечает их как baseline-covered и НЕ переисполняет
-- (директива ниже). Это соответствует ADR-0003 («миграции 001-006 включаются в baseline»)
-- и ADR-0002 (SQL — единственный источник правды; TS-схема — через introspect).
--
-- TODO(Iteration 8): заменить на свежий админский pg_dump --schema-only перед cutover (ADR-0003).
--
-- migrate:baseline-covers-through=6
--
