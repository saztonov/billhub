/**
 * Legacy-маршруты аутентификации (AUTH_MODE=supabase-bridge).
 * Регистрируются диспетчером routes/auth.ts. Поведение НЕ изменено (принцип 1: старый прод).
 * Standalone-путь (раздел 13) — в routes/auth-standalone.ts.
 */
...
async function authRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post<{ Body: LoginBody }>(
    ...
    async (request, reply) => {
      ...
      reply.setCookie(
        ...
        accessTokenCookie(accessTokenExpiresAt),
      );
  ...
  fastify.post('/api/auth/refresh', async (request, reply) => {
    ...
    const { data, error } = await request.server.supabase.auth.refreshSession({
  ...
  fastify.get('/api/auth/me', { preHandler: [authenticate] }, async (request) => {
    ...
    const accessTokenExpiresAt = typeof exp === 'number' ? exp * 1000 : Date.now() + 900 * 1000;
  ...
  fastify.post<{ Body: RegisterBody }>(
    ...
    async (request, reply) => {
      ...
      const { error: insertError } = await request.server.supabase.from('users').insert({
        id: authData.user.id,
        email,
        full_name: fullName,
  ...
  fastify.post<{ Body: ChangePasswordBody }>(
    ...
    async (request, reply) => {
      ...
      const { error: verifyError } = await request.server.supabase.auth.signInWithPassword({
        email: user.email,
      ...
      const { error: updateError } = await request.server.supabase.auth.admin.updateUserById(
        user.id,
        {
  ...
  fastify.post<{ Body: CreateUserBody }>(
    ...
    async (request, reply) => {
      ...
      if (siteIds && siteIds.length > 0) {
        ...
        if (mappingError) {
          request.log.error({ error: mappingError }, 'Ошибка привязки пользователя к объектам');
        }
      }
